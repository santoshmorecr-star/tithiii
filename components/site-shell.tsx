...
export function AdSlot({label='Advertisement', slot}:{label?:string;slot?:string}){
 const adRef=useRef<HTMLElement>(null)
 useEffect(()=>{
  const ad=adRef.current
  if(!ad || !slot || ad.getAttribute('data-adsbygoogle-status')) return
  try{(window.adsbygoogle=window.adsbygoogle||[]).push({})}catch{ /* AdSense may be unavailable in preview or before approval. */}
 },[slot])
 return <div className="border-y border-dashed border-border py-3"><p className="mb-2 text-center text-[10px] uppercase tracking-[.25em] text-muted-foreground/60">{label}</p>{slot?<ins ref={adRef} className="adsbygoogle block min-h-20 w-full" data-ad-client="ca-pub-5893013929731437" data-ad-slot={slot} data-ad-format="auto" data-full-width-responsive="true" />:<p className="text-center text-xs text-muted-foreground">Ad unit not configured yet</p>}</div>
}
